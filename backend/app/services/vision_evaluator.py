...
class VisionEvaluator:
    ...
    def _extract_letter_and_confidence(self, text):
        """Extract letter, confidence, and feedback from the API response,
           validating against known classes.
        
        Args:
            text: Text response from the API
            
        Returns:
            Tuple of (letter, confidence, feedback)
        """
        print("\n--- STARTING LETTER/CONFIDENCE EXTRACTION ---")
        print(f"Raw response text received:\n```\n{text}\n```")
        
        # Handle empty or invalid response
        if not text or not isinstance(text, str):
            print(f"ERROR: Empty or invalid response text type: {type(text)}")
            return "Invalid", 0.0, "Could not process the image response"
            
        print(f"Response text length: {len(text)}")
        
        # Trim whitespace and potential markdown formatting
        ...
        letter, confidence, feedback = "Invalid", 0.0, "Could not parse response"

        # Attempt 1: Parse the cleaned text directly as JSON
        try:
            json_data = json.loads(cleaned_text)
            print(f"Successfully parsed the entire cleaned response as JSON: {json_data}")
            
            # Extract letter, confidence, and feedback from JSON
            extracted_letter = json_data.get("letter", None)
            
            # Validate the extracted letter
            if isinstance(extracted_letter, str) and extracted_letter.upper() in VALID_CLASSES:
                letter = extracted_letter.upper()
                print(f"Validated letter: \'{letter}\'")
            elif extracted_letter is not None:
                print(f"WARNING: Invalid label '{extracted_letter}' received from API. Not in VALID_CLASSES.")
                letter = "Invalid" # Mark as invalid if not in our known set
            else:
                print("WARNING: No letter found in JSON response.")
                letter = "Invalid"
            
            # Normalize confidence
            confidence_raw = json_data.get("confidence", 0.5)
            confidence = 0.0 # Default to 0 if invalid letter
            if letter != "Invalid":
                if isinstance(confidence_raw, (int, float)):
                    ...
                    try:
                        ...
                        print(f"Could not parse confidence string: {confidence_raw}")
                        confidence = 0.5 # Assign default if parsing fails but letter was valid
            
            feedback = json_data.get("feedback", "No feedback provided")
            
            print(f"Extracted from direct JSON: letter=\'{letter}\', confidence={confidence:.2f}, feedback=\'{feedback[:50]}...\'")
            print("--- EXTRACTION SUCCESSFUL (DIRECT JSON) ---")
            return letter, confidence, feedback
            
        except json.JSONDecodeError as e:
            print(f"JSONDecodeError on cleaned text: {str(e)}. Will try regex fallback.")
            
        # Attempt 2: Find JSON embedded within the text using regex
        print("Attempting to find embedded JSON using regex...")
        json_pattern = r'\{(?:[^{}]|(?:\{[^{}]*\}))*\}' # Simplified JSON pattern
        matches = list(re.finditer(json_pattern, cleaned_text))
        print(f"Found {len(matches)} potential JSON matches in the text")
        
        for i, match in enumerate(matches):
            json_str = match.group(0)
            print(f"Attempting to parse JSON match #{i+1}:\n```\n{json_str}\n```")
            try:
                json_data = json.loads(json_str)
                print(f"Successfully parsed embedded JSON match: {json_data}")
                
                if isinstance(json_data, dict) and ("letter" in json_data):
                    extracted_letter = json_data.get("letter", None)
                    
                    # Validate the extracted letter
                    if isinstance(extracted_letter, str) and extracted_letter.upper() in VALID_CLASSES:
                        letter = extracted_letter.upper()
                        print(f"Validated letter from embedded JSON: \'{letter}\'")
                    elif extracted_letter is not None:
                        print(f"WARNING: Invalid label '{extracted_letter}' received in embedded JSON.")
                        continue # Try next match if invalid
                    else:
                        print("WARNING: No letter found in embedded JSON.")
                        continue # Try next match
                    
                    # Normalize confidence
                    confidence_raw = json_data.get("confidence", 0.5)
                    confidence = 0.0 # Default to 0 if invalid letter
                    if letter != "Invalid": # Should always be true here, but for safety
                        if isinstance(confidence_raw, (int, float)):
                            confidence = float(confidence_raw) / 100.0 if confidence_raw > 1 else float(confidence_raw)
                        elif isinstance(confidence_raw, str):
                            try:
                    ...
                    print(f"Extracted from embedded JSON: letter=\'{letter}\', confidence={confidence:.2f}, feedback=\'{feedback[:50]}...\'")
                    print("--- EXTRACTION SUCCESSFUL (EMBEDDED JSON) ---")
                    return letter, confidence, feedback
            except json.JSONDecodeError as e:
                print(f"Failed to parse potential JSON match #{i+1}: {str(e)}")
                continue
                
        # Fallback if no valid JSON found
        print("Could not extract valid JSON data from response.")
        print("--- EXTRACTION FAILED ---")
        return "Invalid", 0.0, cleaned_text[:200] # Return original text snippet as feedback
                ...
                if mode == 'feedback':
                    ...
                    if not feedback or len(feedback) < 20:
                        if expected_sign and letter.upper() == expected_sign.upper():
                            feedback = f"Your sign appears to be correct for '{expected_sign}'."
                        else:
