...
MODEL_CONFIGS = {
    "pro": {
        ...
        "rate_limit_delay": 35,  # 35 seconds between requests (Pro tier rate limits)
        "max_retries": 3
    }
}
...
def get_asl_prediction(image_path: str, prompt_strategy: Literal["zero_shot", "few_shot", "chain_of_thought"] = "zero_shot") -> dict:
    """Get ASL prediction from Gemini 1.5 Pro model for a given image path, including visibility check."""
    config = MODEL_CONFIGS["pro"]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test Gemini 1.5 Pro model for ASL recognition')
    ...
    results_file = f"pro_{args.prompt_strategy}_result_{timestamp}.json"
