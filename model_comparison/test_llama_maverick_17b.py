...
def main():
    ...
    logging.info(f"\nTesting Llama Maverick 17B model with {args.prompt_strategy} strategy...")
    result = get_asl_prediction(image_path, args.prompt_strategy)
    print(f"\nLlama Maverick 17B {args.prompt_strategy} result:")
    print(json.dumps(result, indent=2))
    
    # Save the result to a file
    timestamp = int(time.time())
    results_file = f"llama_maverick_17b_{args.prompt_strategy}_result_{timestamp}.json"
