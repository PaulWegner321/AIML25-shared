...
def get_asl_prediction(image_path: str, prompt_strategy: Literal["zero_shot", "few_shot", "chain_of_thought"] = "zero_shot") -> dict:
    """Get ASL prediction from GPT-4 Turbo Vision model."""
    ...
    try:
        ...
        response_time = time.time() - start_time
        logging.error(f"Error testing GPT-4 Turbo Vision: {e}")
        return {
            "error": str(e),
            "metadata": {
                "response_time_seconds": round(response_time, 3)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Test GPT-4 Turbo Vision model for ASL recognition')
    ...
    # Test the model
    logging.info(f"Testing GPT-4 Turbo Vision model with image: {image_path}")
    result = get_asl_prediction(image_path, args.prompt_strategy)
    
    # Print the result
    print("\nGPT-4 Turbo Vision result:")
    print(json.dumps(result, indent=2))
    
    # Save the result to a file
    timestamp = int(time.time())
    results_file = f"gpt4_turbo_{args.prompt_strategy}_result_{timestamp}.json"
